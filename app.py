...
def fish_tts(key,wav_path,text):
 ...
 body=fld("title","sawti"+str(int(time.time())))+fld("type","tts")
 body+=("--"+boundary+"\r\nContent-Disposition: form-data; name=\"file\"; filename=\"ref.wav\"\r\nContent-Type: audio/wav\r\n\r\n").encode()
 body+=wav+b"\r\n--"+boundary.encode()+b"--\r\n"
 req=urllib.request.Request("https://api.fish.audio/v1/model",data=body,headers={
  "Authorization":"Bearer "+key,"Content-Type":"multipart/form-data; boundary="+boundary})
 vid=urllib.request.urlopen(req,timeout=180).read().decode().strip().strip('"')
