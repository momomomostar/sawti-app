...
<label style="display:block;background:#2a2a3d;border:1px dashed #555;border-radius:10px;padding:12px;text-align:center;cursor:pointer;margin-bottom:10px">
📂 اختر ملف صوتي من الهاتف
<input id="f" type="file" accept="audio/*" style="display:none">
</label>
<div id="fn" style="display:none;background:rgba(80,250,123,.1);color:#50fa7b;padding:8px;border-radius:8px;text-align:center;margin-bottom:10px"></div>
...
<select id="v"></select>
<input id="h" type="text" placeholder="تلميح النبرة (اختياري) — مثال: بالدارجة المغربية، ببطء وبوضوح">
...
document.getElementById('f').onchange=function(){
 ...
 if(!file)return;
 if(file.size>15*1024*1024){msg('m1','الملف كبير بزاف (الحد 15MB)',0);this.value='';return}
 bl=file;
 s.disabled=false;
 var e=document.getElementById('fn');
 e.style.display='block';
 e.textContent='📂 '+file.name+' — جاهز، اضغط «حفظ العينة» ✅';
 msg('m1','تم اختيار الملف ✅',1);
};
s.onclick=async function(){var n=document.getElementById('n').value.trim();
if(!n||!bl){msg('m1','اكتب الاسم وسجّل أولاً ولا اختر ملف',0);return}
...
if(hint){tx='['+hint+'] '+tx}
f.append('name',v.value);f.append('text',tx);f.append('api_key',k.value);
